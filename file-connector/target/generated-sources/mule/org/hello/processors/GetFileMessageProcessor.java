...
import org.hello.adapters.FileConnectorConnectorLifecycleAdapter;
...
/**
 * GetFileMessageProcessor invokes the {@link org.hello.FileConnectorConnector#GetFile(java.lang.String)} method in {@link FileConnectorConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetFileMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(FileConnectorConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.hello.adapters.FileConnectorConnectorConnectionManager"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        FileConnectorConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((FileConnectorConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("GetFile"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((FileConnectorConnectorConnectionManager) moduleObject);
        }
        String _transformedUsername = null;
        String _transformedPassword = null;
        FileConnectorConnectorLifecycleAdapter connection = null;
        try {
            if (username!= null) {
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, GetFileMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (_castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("GetFile"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, GetFileMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, _castedModuleObject.getUsername()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, GetFileMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("GetFile"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, GetFileMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedPath = ((String) evaluateAndTransform(_muleMessage, GetFileMessageProcessor.class.getDeclaredField("_pathType").getGenericType(), null, path));
            ...
            connection = _castedModuleObject.acquireConnection(new FileConnectorConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword));
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("GetFile"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.GetFile(_transformedPath);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("GetFile"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
                    ...
                    _castedModuleObject.releaseConnection(new FileConnectorConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("GetFile"), event, e);
            }
