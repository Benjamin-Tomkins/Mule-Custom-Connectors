...
/**
 * ReadBarCodeMessageProcessor invokes the {@link com.cts.qr.QRCodeConnector#ReadBarCode(java.lang.String)} method in {@link QRCodeConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class ReadBarCodeMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(ReadBarCodeMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("ReadBarCode"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, ReadBarCodeMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (_castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("ReadBarCode"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, ReadBarCodeMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, _castedModuleObject.getUsername()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, ReadBarCodeMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("ReadBarCode"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, ReadBarCodeMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedFilePath = ((String) evaluateAndTransform(_muleMessage, ReadBarCodeMessageProcessor.class.getDeclaredField("_filePathType").getGenericType(), null, filePath));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("ReadBarCode"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.ReadBarCode(_transformedFilePath);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("ReadBarCode"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
