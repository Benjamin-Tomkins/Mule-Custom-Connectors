...
import com.cts.qr.adapters.QRCodeConnectorConnectionManager;
import com.cts.qr.adapters.QRCodeConnectorLifecycleAdapter;
...
/**
 * CreateBarCodeMessageProcessor invokes the {@link com.cts.qr.QRCodeConnector#createBarCode(java.lang.String, java.lang.String)} method in {@link QRCodeConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class CreateBarCodeMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _BarCodeDataType;
    ...
    private static Logger logger = LoggerFactory.getLogger(CreateBarCodeMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(QRCodeConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("com.cts.qr.adapters.QRCodeConnectorConnectionManager"), e, this);
            }
        }
     ...
     * @param value Value to set
     */
    public void setBarCodeData(Object value) {
        ...
        throws MuleException
    {
        ...
        QRCodeConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((QRCodeConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createBarCode"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((QRCodeConnectorConnectionManager) moduleObject);
        }
        String _transformedUsername = null;
        String _transformedPassword = null;
        QRCodeConnectorLifecycleAdapter connection = null;
        try {
            if (username!= null) {
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (_castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createBarCode"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, _castedModuleObject.getUsername()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createBarCode"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedBarCodeData = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_BarCodeDataType").getGenericType(), null, BarCodeData));
            String _transformedFilePath = ((String) evaluateAndTransform(_muleMessage, CreateBarCodeMessageProcessor.class.getDeclaredField("_filePathType").getGenericType(), null, filePath));
            ...
            connection = _castedModuleObject.acquireConnection(new QRCodeConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword));
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createBarCode"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.createBarCode(_transformedBarCodeData, _transformedFilePath);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("createBarCode"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
                    ...
                    _castedModuleObject.releaseConnection(new QRCodeConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("createBarCode"), event, e);
            }
