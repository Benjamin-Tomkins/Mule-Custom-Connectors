...
package com.kiss.couch.processors;
...
import com.kiss.couch.CouchDBConnector;
import com.kiss.couch.adapters.CouchDBConnectorConnectionManager;
import com.kiss.couch.adapters.CouchDBConnectorLifecycleAdapter;
...
/**
 * MyProcessorMessageProcessor invokes the {@link com.kiss.couch.CouchDBConnector#myProcessor(java.lang.String)} method in {@link CouchDBConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(CouchDBConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("com.kiss.couch.adapters.CouchDBConnectorConnectionManager"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        CouchDBConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((CouchDBConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            ...
            _castedModuleObject = ((CouchDBConnectorConnectionManager) moduleObject);
        }
        String _transformedUsername = null;
        String _transformedPassword = null;
        CouchDBConnectorLifecycleAdapter connection = null;
        try {
            if (username!= null) {
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (_castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("myProcessor"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, _castedModuleObject.getUsername()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("myProcessor"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedContent = ((String) evaluateAndTransform(_muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_contentType").getGenericType(), null, content));
            ...
            connection = _castedModuleObject.acquireConnection(new CouchDBConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword));
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("myProcessor"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.myProcessor(_transformedContent);
            ...
            try {
                if (connection!= null) {
                    ...
                    _castedModuleObject.releaseConnection(new CouchDBConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("myProcessor"), event, e);
            }
