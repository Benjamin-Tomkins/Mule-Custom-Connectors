...
package com.cts.instagram.processors;
...
import com.cts.instagram.adapters.InstagramConnectorConnectionManager;
import com.cts.instagram.adapters.InstagramConnectorLifecycleAdapter;
...
/**
 * StartInstagramMessageProcessor invokes the {@link com.cts.instagram.InstagramConnector#StartInstagram(java.lang.String)} method in {@link InstagramConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class StartInstagramMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(StartInstagramMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(InstagramConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("com.cts.instagram.adapters.InstagramConnectorConnectionManager"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        InstagramConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((InstagramConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("StartInstagram"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((InstagramConnectorConnectionManager) moduleObject);
        }
        String _transformedUsername = null;
        String _transformedPassword = null;
        InstagramConnectorLifecycleAdapter connection = null;
        try {
            if (username!= null) {
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, StartInstagramMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (_castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("StartInstagram"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                _transformedUsername = ((String) evaluateAndTransform(_muleMessage, StartInstagramMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, _castedModuleObject.getUsername()));
            }
            if (password!= null) {
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, StartInstagramMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (_castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("StartInstagram"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                _transformedPassword = ((String) evaluateAndTransform(_muleMessage, StartInstagramMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, _castedModuleObject.getPassword()));
            }
            String _transformedStart = ((String) evaluateAndTransform(_muleMessage, StartInstagramMessageProcessor.class.getDeclaredField("_startType").getGenericType(), null, start));
            ...
            connection = _castedModuleObject.acquireConnection(new InstagramConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword));
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("StartInstagram"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.StartInstagram(_transformedStart);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("StartInstagram"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
                    ...
                    _castedModuleObject.releaseConnection(new InstagramConnectorConnectionManager.ConnectionKey(_transformedUsername, _transformedPassword), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("StartInstagram"), event, e);
            }
